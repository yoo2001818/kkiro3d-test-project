  ...
  registerEvents() {
    ...
    this.node.addEventListener('mousemove', e => {
      if (document.pointerLockElement || document.mozPointerLockElement) {
        ...
        if (camera.fpsController == null) return;
        this.engine.actions.external.execute('fpsController.addRotation',
          camera, -e.movementY / 400, e.movementX / 400);
        return;
      }
  ...
  update(delta) {
    ...
    if (!this.node.requestPointerLock && !this.node.mozRequestPointerLock) {
      if (Math.abs(this.mouseX) > 10 || Math.abs(this.mouseY) > 10) {
        this.engine.actions.external.execute('fpsController.addRotation',
          camera, -Math.sin(this.mouseY / 4000), Math.sin(this.mouseX / 4000));
      }
    }
    ...
    if (velocity[0] !== 0 || velocity[1] !== 0 || velocity[2] !== 0) {
      this.engine.actions.external.execute('fpsController.move', camera,
        velocity, delta);
    }
  }
}
